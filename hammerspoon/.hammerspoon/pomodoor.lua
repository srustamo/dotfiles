...
local pom_work_period_sec  = 65 * 60
-- local pom_work_period_sec  = 1 * 60
...
local function pom_update_display()
  ...
  local str = string.format ("[%s|%02d:|#%02d]", pom_curr_active_type, time_min, pom_work_count)
  ...
  if pom_is_active == false then
    ...
    if (pom_time_left <= 0 ) then
      ...
      if pom_curr_active_type == "work" then 
        hs.alert.show("Work Complete!", 300)
